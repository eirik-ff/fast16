...
namespace fast16
{
  ...
  inline int fast16_corner_score_10(const fast16_px *cache_0, const int offset[], int b)
  {
    b++;
  ...
  void fast16_corner_score_10(
      const fast16_px *img,
      const int img_stride,
      const std::vector<fast16_xy> &corners,
      const int threshold,
      std::vector<int> &scores)
  {
    ...
    for (unsigned int n = 0; n < corners.size(); n++)
      scores[n] = fast16_corner_score_10(img + corners[n].y * img_stride + corners[n].x, pixel, threshold);
  }
