...
namespace fast16
{
  ...
  void fast16_corner_detect_10(const fast16_px *img, int img_width, int img_height, int img_stride,
                               short barrier, std::vector<fast16_xy> &corners)
  {
    int y, cb, c_b;
    const fast16_px *line_max, *line_min;
    const fast16_px *cache_0;
    ...
    for (y = 3; y < img_height - 3; y++)
    {
      ...
      success:
        corners.push_back(fast16_xy(static_cast<short>(cache_0 - line_min),
                                    static_cast<short>(y)));
      }
    }
  }
